...
function Try-FastPath([string]$kind,[string]$shape,[string]$folder,[object[]]$local,[object[]]$global){
    ...
    $exLocal = @($local | Where-Object {
        $s = if($_.Shape){ $_.Shape } else { $_.Name }
        (Get-NormKey $_.Folder $s) -eq $needKey
    })
    if($exLocal.Count -gt 0){
        $best = $exLocal[0]
        $bestName = if($best.Shape){ $best.Shape } else { $best.Name }
        Write-Log 'INFO' ("FastPath hit: LocalExact for '{0}' in '{1}' -> {2}/{3}" -f $shape,$folder,$bestName,$best.Folder)
        return $best
    }

    # 2) Exact in global
    $exGlob = @($global | Where-Object {
        $s = if($_.Shape){ $_.Shape } else { $_.Name }
        (Get-NormKey $_.Folder $s) -eq $needKey
    })
    if($exGlob.Count -gt 0){
        $best = $exGlob[0]
        $bestName = if($best.Shape){ $best.Shape } else { $best.Name }
        Write-Log 'INFO' ("FastPath hit: GlobalExact for '{0}' in '{1}' -> {2}/{3}" -f $shape,$folder,$bestName,$best.Folder)
        return $best
    }

    # 3) 2-token overlap in same folder
    $twoLocal = @($local | Where-Object {
        $s = if($_.Shape){ $_.Shape } else { $_.Name }
        TwoTokenOverlap $needTokens (Get-TrimmedTokens $_.Folder $s)
    })
    if($twoLocal.Count -gt 0){
        $best = ($twoLocal | Sort-Object { - [double](Score-Candidate $shape $folder $_) })[0]
        $bestName = if($best.Shape){ $best.Shape } else { $best.Name }
        Write-Log 'INFO' ("FastPath hit: Local2Tok for '{0}' in '{1}' -> {2}/{3}" -f $shape,$folder,$bestName,$best.Folder)
        return $best
    }

    # 4) 2-token overlap in global
    $twoGlob = @($global | Where-Object {
        $s = if($_.Shape){ $_.Shape } else { $_.Name }
        TwoTokenOverlap $needTokens (Get-TrimmedTokens $_.Folder $s)
    })
    if($twoGlob.Count -gt 0){
        ...
        $bestName = if($best.Shape){ $best.Shape } else { $best.Name }
        Write-Log 'INFO' ("FastPath hit: Global2Tok for '{0}' in '{1}' -> {2}/{3}" -f $shape,$folder,$bestName,$best.Folder)
        return $best
    }
