...
class ConsistEditorGUI:
    ...
    def _detect_virtual_environment(self):
        try:
            if hasattr(sys, 'real_prefix') or (hasattr(sys, 'base_prefix') and sys.base_prefix != sys.prefix):
                ...
                self.log_message(
                    "Virtual environment detected and will be used for resolver")
            else:
                script_dir = Path(__file__).parent if __file__ else Path.cwd()
                for v in ['venv', '.venv', 'env', '.env', 'virtualenv']:
                    ...
                    if vp.is_dir():
                        ...
                        if py.exists():
                            ...
                            self.log_message(
                                f"Found virtual environment at: {vp}")
                            break
            ...
            self.venv_python_path = sys.executable
            self.log_message(
                f"Error detecting virtual environment: {e}, using system Python")
        self.setup_gui()
    ...
    def setup_gui(self):
        ...
        title_label = ttk.Label(
            main_frame, text="MSTS Consist Editor - TSRE5 Style", font=('Arial', 16, 'bold'))
        title_label.grid(row=0, column=0, columnspan=3, pady=(0, 20))

        left_panel = ttk.LabelFrame(
            main_frame, text="File Selection & Controls", padding="10")
        left_panel.grid(row=1, column=0, sticky=(
            tk.W, tk.E, tk.N, tk.S), padx=(0, 10))
        self.setup_file_selection(left_panel)
        self.setup_controls(left_panel)

        right_panel = ttk.Frame(main_frame)
        right_panel.grid(row=1, column=1, rowspan=2,
                         sticky=(tk.W, tk.E, tk.N, tk.S))
        right_panel.columnconfigure(0, weight=1)
        right_panel.rowconfigure(1, weight=1)

        viewer_frame = ttk.LabelFrame(
            right_panel, text="Consist Viewer", padding="10")
        viewer_frame.grid(row=0, column=0, sticky=(
        ...
        output_frame = ttk.LabelFrame(
            right_panel, text="Output & Status", padding="10")
    ...
    def setup_file_selection(self, parent):
        ttk.Label(parent, text="Consists Directory:").grid(
            row=0, column=0, sticky=tk.W, pady=(0, 5))
        c_frame = ttk.Frame(parent)
        c_frame.grid(row=1, column=0, sticky=(tk.W, tk.E), pady=(0, 10))
        c_frame.columnconfigure(0, weight=1)
        ttk.Entry(c_frame, textvariable=self.consists_path, width=40).grid(
            row=0, column=0, sticky=(tk.W, tk.E), padx=(0, 5))
        ttk.Button(c_frame, text="Browse",
                   command=self.browse_consists_folder).grid(row=0, column=1)

        ttk.Label(parent, text="Trainset Directory:").grid(
            row=2, column=0, sticky=tk.W, pady=(0, 5))
        t_frame = ttk.Frame(parent)
        t_frame.grid(row=3, column=0, sticky=(tk.W, tk.E), pady=(0, 10))
        t_frame.columnconfigure(0, weight=1)
        ttk.Entry(t_frame, textvariable=self.trainset_path, width=40).grid(
            row=0, column=0, sticky=(tk.W, tk.E), padx=(0, 5))
        ttk.Button(t_frame, text="Browse",
                   command=self.browse_trainset_folder).grid(row=0, column=1)

        load_frame = ttk.Frame(parent)
        load_frame.grid(row=4, column=0, sticky=(tk.W, tk.E), pady=(10, 0))
        load_frame.columnconfigure(0, weight=1)
        self.load_button = ttk.Button(
            load_frame, text="Load & Analyze Consists", command=self.load_and_analyze)
        self.load_button.grid(row=0, column=0, pady=10)

        files_frame = ttk.LabelFrame(parent, text="Consist Files", padding="6")
        files_frame.grid(row=5, column=0, sticky=(
        ...
        self.consist_files_tree.grid(
            row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        files_scroll = ttk.Scrollbar(
            files_frame, orient='vertical', command=self.consist_files_tree.yview)
        self.consist_files_tree.configure(yscrollcommand=files_scroll.set)
        files_scroll.grid(row=0, column=1, sticky=(tk.N, tk.S))
        self.consist_files_tree.bind(
            '<<TreeviewSelect>>', self.on_consist_file_selected)
        self.consist_files_tree.tag_configure(
            'missing', foreground=self.colors['missing'])
        self.consist_files_tree.tag_configure(
            'no_missing', foreground=self.colors['existing'])
        self.consist_files_tree.tag_configure('error', foreground='#A52A2A')

    def setup_controls(self, parent):
        ttk.Separator(parent, orient='horizontal').grid(
            row=6, column=0, sticky=(tk.W, tk.E), pady=20)

        controls_frame = ttk.LabelFrame(
            parent, text="Resolver Options", padding="10")
        ...
        ttk.Checkbutton(controls_frame, text="Dry Run (Preview only)",
                        variable=self.dry_run_var).grid(row=0, column=0, sticky=tk.W)
        ttk.Checkbutton(controls_frame, text="Explain (Detailed info)",
                        variable=self.explain_var).grid(row=1, column=0, sticky=tk.W)
        ttk.Checkbutton(controls_frame, text="Debug Mode", variable=self.debug_var).grid(
            row=2, column=0, sticky=tk.W)

        mode_frame = ttk.Frame(controls_frame)
        mode_frame.grid(row=3, column=0, sticky=(tk.W, tk.E), pady=(10, 0))
        ttk.Label(mode_frame, text="Resolve:").grid(
            row=0, column=0, sticky=tk.W)
        ttk.Radiobutton(mode_frame, text="Selected file only", variable=self.resolve_mode_var,
                        value='selected').grid(row=0, column=1, sticky=tk.W, padx=(10, 0))
        ttk.Radiobutton(mode_frame, text="All files in directory", variable=self.resolve_mode_var,
                        value='all').grid(row=1, column=1, sticky=tk.W, padx=(10, 0))

        buttons_frame = ttk.Frame(parent)
        buttons_frame.grid(row=8, column=0, sticky=(tk.W, tk.E), pady=10)
        buttons_frame.columnconfigure(0, weight=1)
        buttons_frame.columnconfigure(1, weight=1)
        self.resolve_button = ttk.Button(
            buttons_frame, text="Run Resolver", command=self.run_resolver, state='disabled')
        self.resolve_button.grid(row=0, column=0, padx=(
            0, 5), pady=2, sticky=(tk.W, tk.E))
        self.refresh_button = ttk.Button(
            buttons_frame, text="Refresh View", command=self.refresh_consist_view)
        self.refresh_button.grid(row=0, column=1, padx=(
            5, 0), pady=2, sticky=(tk.W, tk.E))
        self.resolve_mode_var.trace_add(
            'write', self._update_resolve_button_text)
        self.refresh_counts_button = ttk.Button(
            buttons_frame, text="Refresh Counts", command=self.refresh_counts)
        self.refresh_counts_button.grid(
            row=1, column=0, columnspan=2, pady=(6, 0), sticky=(tk.W, tk.E))

        self.scan_status_label = ttk.Label(parent, text='')
        self.scan_status_label.grid(
            row=10, column=0, sticky=(tk.W), pady=(4, 0))
        self.resolver_progress = ttk.Progressbar(
            parent, orient='horizontal', length=400, mode='determinate', variable=self.resolver_progress_var)

    def setup_consist_viewer(self, parent):
        columns = ('Type', 'Folder', 'Name', 'Status')
        self.consist_tree = ttk.Treeview(
        ...
        self.consist_tree.tag_configure(
            'missing', foreground=self.colors['missing'])
        self.consist_tree.tag_configure(
            'existing', foreground=self.colors['existing'])
        self.consist_tree.tag_configure(
            'unresolved', foreground=self.colors['unresolved'])
        self.consist_tree.tag_configure(
            'changed', foreground=self.colors['changed'])
        self.consist_tree.tag_configure('unknown', foreground='#666666')

        tree_scroll_v = ttk.Scrollbar(
            parent, orient='vertical', command=self.consist_tree.yview)
        tree_scroll_h = ttk.Scrollbar(
            parent, orient='horizontal', command=self.consist_tree.xview)
        self.consist_tree.configure(
            yscrollcommand=tree_scroll_v.set, xscrollcommand=tree_scroll_h.set)
        self.consist_tree.grid(
            row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        tree_scroll_v.grid(row=0, column=1, sticky=(tk.N, tk.S))
        tree_scroll_h.grid(row=1, column=0, sticky=(tk.W, tk.E))

        parent.columnconfigure(2, weight=0)
        stores_frame = ttk.LabelFrame(
            parent, text="Stores (Engines / Wagons)", padding="6")
        stores_frame.grid(row=0, column=2, rowspan=4,
                          sticky=(tk.N, tk.S, tk.E), padx=(10, 0))
        ...
        self.store_subfolder_cb = ttk.Combobox(
            stores_frame, textvariable=self.store_subfolder_var, values=[''], state='readonly', width=20)
        self.store_subfolder_cb.grid(row=0, column=2, padx=(6, 0), pady=(0, 6))
        self.store_subfolder_cb.bind(
            '<<ComboboxSelected>>', lambda e: self.load_store_items())

        ttk.Checkbutton(stores_frame, text='Scan all top-level subfolders', variable=self.scan_all_subfolders_var,
                        command=self.load_store_items).grid(row=0, column=3, padx=(6, 0))

        store_filter = ttk.Combobox(stores_frame, textvariable=self.store_filter_var, values=[
                                    'All', 'Engines', 'Wagons'], state='readonly', width=12)
        store_filter.grid(row=0, column=0, columnspan=2, pady=(0, 6))
        store_filter.bind('<<ComboboxSelected>>',
                          lambda e: self.load_store_items())

        ttk.Label(stores_frame, text='Search:').grid(
            row=1, column=0, sticky=tk.W, pady=(6, 0))
        search_entry = ttk.Entry(
            stores_frame, textvariable=self.store_search_var, width=20)
        ...
        list_area.grid(row=2, column=0, columnspan=3,
        ...
        self.store_listbox = tk.Listbox(
            list_area, height=20, exportselection=False)  # no width param
        self.store_listbox.grid(
            row=0, column=0, sticky=(tk.N, tk.S, tk.E, tk.W))
        store_scroll = ttk.Scrollbar(
            list_area, orient='vertical', command=self.store_listbox.yview)
        ...
        self.store_message_label.grid(
            row=3, column=0, columnspan=3, pady=(6, 0))

        ttk.Button(stores_frame, text='Refresh Stores', command=self._refresh_store_cache).grid(
            row=6, column=0, columnspan=2, pady=(8, 0))

        self.store_scan_label_var = tk.StringVar(value='')
        ttk.Label(stores_frame, textvariable=self.store_scan_label_var).grid(
            row=15, column=0, columnspan=3, pady=(6, 0))

        self.store_progress_var = tk.DoubleVar(value=0.0)
        self.store_progress = ttk.Progressbar(
            stores_frame, orient='horizontal', length=200, mode='determinate', variable=self.store_progress_var)
        self._store_progress_visible = False

        ttk.Label(stores_frame, text='Num to add:').grid(
            row=4, column=0, sticky=tk.W, pady=(6, 0))
        self.add_number_var = tk.StringVar(value='1')
        ttk.Entry(stores_frame, textvariable=self.add_number_var, width=6).grid(
            row=4, column=1, sticky=tk.W, pady=(6, 0))

        btn_frame = ttk.Frame(stores_frame)
        btn_frame.grid(row=5, column=0, columnspan=2, pady=(8, 0))
        ttk.Button(btn_frame, text='Add Beg', command=lambda: self.insert_store_item(
            'beg')).grid(row=0, column=0, padx=2, pady=2)
        ttk.Button(btn_frame, text='Add Cur', command=lambda: self.insert_store_item(
            'cur')).grid(row=0, column=1, padx=2, pady=2)
        ttk.Button(btn_frame, text='Add End', command=lambda: self.insert_store_item(
            'end')).grid(row=0, column=2, padx=2, pady=2)
        ttk.Button(btn_frame, text='Add N', command=lambda: self.insert_store_item(
            'at')).grid(row=0, column=3, padx=2, pady=2)

        ttk.Label(stores_frame, text='Replace with:').grid(
            row=9, column=0, sticky=tk.W, pady=(6, 0))
        self.store_replace_var = tk.StringVar()
        self.store_replace_cb = ttk.Combobox(
            stores_frame, textvariable=self.store_replace_var, state='readonly', width=30)
        self.store_replace_cb.grid(
            row=9, column=1, columnspan=2, sticky=(tk.W, tk.E), pady=(6, 0))
        self.store_replace_cb['values'] = []

        action_frame = ttk.Frame(stores_frame)
        action_frame.grid(row=11, column=0, columnspan=3, pady=(8, 0))
        ttk.Button(action_frame, text='Move Up', command=self.move_selected_up).grid(
            row=0, column=0, padx=2, pady=2)
        ttk.Button(action_frame, text='Move Down', command=self.move_selected_down).grid(
            row=0, column=1, padx=2, pady=2)
        ttk.Button(action_frame, text='Replace', command=self.replace_selected_with).grid(
            row=0, column=2, padx=2, pady=2)
        self.save_button = ttk.Button(
            action_frame, text="Save As", command=self.save_current_consist, state='disabled')
        self.save_button.grid(row=0, column=3, padx=2, pady=2)
        self.delete_button = ttk.Button(
            action_frame, text="Delete", command=self.delete_selected_entry, state='disabled')
            ...
            def _debounced_update(*args):
                try:
                    ...
                    def _delayed():
                        self.load_store_items()
                        self.update_store_subfolders()
                    self._trainset_update_after_id = self.root.after(
                        300, _delayed)
                except Exception:
                    try:
                        self.load_store_items()
        ...
        missing_frame = ttk.LabelFrame(
            parent, text="Missing Items (Selected Consist)", padding="6")
        missing_frame.grid(row=3, column=0, columnspan=2,
                           sticky=(tk.W, tk.E, tk.N, tk.S), pady=(10, 0))
        missing_frame.columnconfigure(0, weight=1)
        self.missing_text = scrolledtext.ScrolledText(
            missing_frame, height=8, wrap=tk.WORD)
        self.missing_text.grid(
            row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        self.missing_text.insert(
            tk.END, 'Select a consist file to view missing items.')
        self.missing_text.config(state='disabled')

    def setup_output_area(self, parent):
        self.output_text = scrolledtext.ScrolledText(
            parent, height=20, wrap=tk.WORD)
        self.output_text.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        self.log_message("MSTS Consist Editor Tool - Ready")
        self.log_message(
            "Select consists directory and trainset directory, then click 'Load & Analyze'")
        if not self.resolver_script_path:
            self.log_message(
                "WARNING: consistEditor.py not found in current directory")
            self.log_message("Please ensure the resolver script is available")
        if self.venv_python_path != sys.executable:
            self.log_message(
                "Virtual environment detected - resolver will use venv Python")
        else:
            self.log_message(
                "Using system Python for resolver (no virtual environment detected)")
    ...
    def load_and_analyze(self):
        ...
        if not consists_path.exists():
            messagebox.showerror(
                "Error", f"Consists directory not found: {consists_dir}")
            return

        consist_files = list(consists_path.glob("*.con"))
        if not consist_files:
            messagebox.showwarning(
                "Warning", f"No .con files found in: {consists_dir}")
            return

        self.log_message(
            f"Found {len(consist_files)} consist files in: {consists_dir}")
            ...
            for cf in files:
                ...
                try:
                    ...
                    if self.trainset_path.get():
                        ...
                        for e in entries:
                            asset_path = trainset_path / \
                                e['folder'] / f"{e['name']}.{e['extension']}"
        ...
        threading.Thread(target=worker, args=(
            consist_files,), daemon=True).start()
    ...
    def save_current_consist(self):
        ...
        try:
            ...
            if sel:
                ...
                file_path = filedialog.asksaveasfilename(defaultextension='.con', filetypes=[
                                                         ('Consist files', '*.con'), ('All files', '*.*')])
                if not file_path:
                    return
            ...
            def generate_merged_content(orig_content, entries):
                ...
                while entry_index < len(entries):
                    entry = entries[entry_index]
                    kind_block = 'Engine' if entry.get(
                        'type', '').lower().startswith('e') else 'Wagon'
                    name = entry.get('name', '')
            ...
            def process_block(block_lines, entry):
                ...
                for line in block_lines:
                    ...
                    if data_type.lower() in line.lower():
                        ...
                        folder = entry.get('folder', '')
                        result.append(
                            f"{indent}{data_type} ( {name} \"{folder}\" )")
                    else:
                ...
                try:
                    ...
                    if content:
                        try:
                            import re
                            match = re.search(
                                r'TrainCfg\s*\(\s*"([^"]+)"', content, re.IGNORECASE)
                            if match:
        ...
        try:
            ...
            for pat, kind_group, name_group, folder_group in patterns:
                for match in re.finditer(pat, content, flags=re.IGNORECASE | re.DOTALL):
                    all_matches.append(
                        (match.start(), match, kind_group, name_group, folder_group))
            ...
            for start_pos, match, kind_group, name_group, folder_group in all_matches:
                try:
                    if kind_group is not None:
                        ...
                        search_span = content[max(
                            0, match.start() - 400):match.start()]
            ...
            if not entries:
                for line in content.splitlines():
                    ...
                    if not line or line.startswith('//') or line.startswith('#'):
                        continue
                    m = re.search(
                        r'(EngineData|WagonData)\s*\(\s*([^\s\)]+)\s+"?([^"\)]+)"?\s*\)', line, flags=re.IGNORECASE)
                    if m:
                        entry_type = 'Engine' if m.group(
                            1).lower().startswith('e') else 'Wagon'
    ...
    def analyze_single_consist(self, file_path):
        ...
        try:
            ...
            self.log_message(
                f"Loaded {len(entries)} entries from {Path(file_path).name}")

        except Exception as e:
            self.log_message(f"Error analyzing consist file: {e}")
            messagebox.showerror(
                "Error", f"Failed to analyze consist file:\n{str(e)}")
    ...
    def load_store_items(self):
        ...
        store_filter = self.store_filter_var.get() if hasattr(
            self, 'store_filter_var') else 'All'
        ...
        if ts:
            try:
                ...
                sub = ''
                try:
                    sub = self.store_subfolder_var.get() if hasattr(
                        self, 'store_subfolder_var') else ''
                except Exception:
                    sub = ''

                # Cache key includes selected subfolder, scan-all flag, and filter so scans differ
                scan_all = bool(self.scan_all_subfolders_var.get()) if hasattr(
                    self, 'scan_all_subfolders_var') else False
                ...
                if self._store_cache is not None and self._store_cache_trainset == cache_key:
                    ...
                    if ts_path.exists():
                        ...
                        if scan_all:
                            ...
                            threading.Thread(target=self._load_store_items_bg, args=(
                                ts, store_filter, cache_key), daemon=True).start()
                            ...
                            try:
                                self.store_message_label.config(
                                    text='Scanning trainset...')
                            except Exception:
                                pass
                            return
                        patterns = []
                            ...
                            for child in sorted(Path(ts).iterdir()):
                                ...
                                for pat in patterns:
                                    for p in child.glob(pat):
                                        ...
                                        item = {
                                            'display': display, 'folder': folder, 'name': name, 'extension': ext}
        ...
        try:
            if not self.trainset_path.get():
                self.store_message_label.config(
                    text='Select a trainset directory to populate the stores.')
            elif not self.store_items:
                self.store_message_label.config(
                    text='No assets found in the selected trainset.')
            else:
                ...
                try:
                    ...
                    try:
                        self.message_queue.put(
                            ('store_scan_progress', (processed_files, total_files, display)))
                    except Exception:
                        pass
                except Exception:
                    ...
                    try:
                        processed_files += 1
                        self.message_queue.put(
                            ('store_scan_progress', (processed_files, total_files, '')))
                    except Exception:
                        pass
    ...
    def update_store_subfolders(self):
        ...
        try:
            ...
            try:
                ...
                cur = self.store_subfolder_var.get() if hasattr(
                    self, 'store_subfolder_var') else ''
    ...
    def move_selected_up(self):
        ...
        try:
            ...
            if idx <= 0:
                return
            # swap
            self.current_entries[idx -
                                 1], self.current_entries[idx] = self.current_entries[idx], self.current_entries[idx-1]
    ...
    def replace_selected_with(self):
        ...
        try:
            ...
            if not found:
                messagebox.showwarning(
                    'Warning', f'Replacement item not found: {repl}')
                return
        ...
        try:
            ...
            if mode == 'beg':
                ...
                pos = simpledialog.askinteger(
                    'Insert Position', 'Enter 1-based position to insert at', minvalue=1, maxvalue=max(1, len(self.current_entries)+1))
                if pos is None:
                    return
                insert_index = pos - 1
            else:
    ...
    def refresh_consist_tree_from_current_entries(self):
        ...
        try:
            ...
            for i, entry in enumerate(self.current_entries):
                ...
                display_text = f"{entry['folder']}/{entry['name']}.{entry['extension']}" if entry.get(
                    'folder') else f"{entry['name']}.{entry['extension']}"
                ...
                item = self.consist_tree.insert('', 'end', iid=iid, text=display_text,
                                                values=(
                                                    entry['type'], entry['folder'], entry['name'], status),
    ...
    def update_status_summary(self):
        ...
        if self.trainset_path.get():
            ...
            for entry in self.current_entries:
                asset_path = trainset_path / \
                    entry['folder'] / f"{entry['name']}.{entry['extension']}"
                if asset_path.exists():
                    existing += 1
                else:
                    missing += 1
        ...
        # Will be updated after resolver runs
        self.status_labels['changed'].config(text=f"Changed: 0")
    ...
    def update_missing_items_display(self, file_path=None):
        ...
        try:
                ...
                for entry in entries:
                    ...
                    if not asset_path.exists():
                        missing_items.append(
                            f"{entry['folder']}/{entry['name']}.{entry['extension']}")
            ...
            if missing_items:
                self.missing_text.insert(
                    tk.END, f"Missing items in {Path(file_path).name}:\n\n")
                ...
                self.missing_text.insert(
                    tk.END, f"No missing items found in {Path(file_path).name}")
            ...
            self.missing_text.delete(1.0, tk.END)
            self.missing_text.insert(
                tk.END, f"Error loading missing items: {str(e)}")
            self.missing_text.config(state='disabled')
    ...
    def refresh_consist_view(self):
        ...
        if hasattr(self, 'current_consist_file') and self.current_consist_file:
            self.log_message(
                f"Refreshing view for current file: {self.current_consist_file}")
        ...
        if single_consist:
            self.log_message(
                f"Refreshing view for selected file: {single_consist}")
            ...
            if consists_dir:
                ...
                if consist_files:
                    self.log_message(
                        f"Refreshing view for first file: {consist_files[0]}")
    ...
    def refresh_counts(self):
        ...
        if not consists_path.exists():
            messagebox.showwarning(
                'Warning', f'Consists directory not found: {consists_dir}')
            return
                    ...
                    if msg_type == 'log':
                        ...
                        try:
                            if not self.resolver_progress_visible:
                                self.resolver_progress.grid(
                                    row=13, column=0, sticky=(tk.W, tk.E), pady=(4, 0))
                                self.resolver_progress_visible = True
                                self.resolver_progress.config(
                                    mode='indeterminate')
                                self.resolver_progress.start()
                                self.scan_status_label.config(
                                    text='Starting resolver...')
                        except Exception:
                            pass
                        ...
                        try:
                            # clear existing
                            self.consist_files_tree.delete(
                                *self.consist_files_tree.get_children())
                            first_iid = None
                            # reset errors map
                            ...
                            for path_str, display_name, missing_count, err in results:
                                ...
                                display_missing = missing_count if not (isinstance(
                                    missing_count, int) and missing_count == -1) else 'ERR'
                                if display_missing == 'ERR':
                                    tag = 'error'
                                else:
                                    tag = 'missing' if (isinstance(
                                        missing_count, int) and missing_count > 0) else 'no_missing'

                                self.consist_files_tree.insert('', 'end', iid=path_str, values=(
                                    display_missing,), text=display_name, tags=(tag,))
                            ...
                            self.consist_files_tree.tag_configure(
                                'no_missing', foreground=self.colors['existing'])
                            self.consist_files_tree.tag_configure(
                                'error', foreground='#A52A2A')

                            # bind tooltip events for error items
                            self.consist_files_tree.bind(
                                '<Motion>', self._on_tree_motion)
                            self.consist_files_tree.bind(
                            ...
                            if first_iid:
                                try:
                                    self.consist_files_tree.selection_set(
                                        first_iid)
                            ...
                            self.log_message(
                                f"Error updating consist files list: {e}")
                        ...
                        try:
                            ...
                            if not self._store_progress_visible:
                                try:
                                    # show progressbar at row 14, column 0, columnspan 1 to avoid overlap
                                    self.store_progress.grid(
                                        row=14, column=0, columnspan=1, pady=(6, 0))
                                    self._store_progress_visible = True
                                except Exception:
                                    pass
                            try:
                                # switch to determinate and update percent and label
                                pct = int((processed / total) *
                                          100) if total else 100
                            except Exception:
                                pct = 0
                            try:
                                self.store_progress.configure(
                                    mode='determinate')
                            ...
                            try:
                                ...
                                if hasattr(self, 'store_scan_label_var') and current_name:
                                    self.store_scan_label_var.set(
                                        f"Scanning: {current_name} ({processed}/{total})")
                            except Exception:
                                pass
                        except Exception:
                            pass
                        ...
                        try:
                            if hasattr(self, 'current_consist_file') and self.current_consist_file:
                                self.log_message(
                                    f"Refreshing consist viewer for updated file: {self.current_consist_file}")
                                self.analyze_single_consist(
                                    self.current_consist_file)
                        except Exception as e:
                            self.log_message(
                                f"Error refreshing current consist after resolver: {e}")
    ...
    def _on_tree_motion(self, event):
        ...
        try:
            ...
            lbl = ttk.Label(tw, text=err, background='#FFFACD',
    ...
    def run_resolver(self):
        ...
        if not consists_dir or not trainset_dir:
            messagebox.showerror(
                "Error", "Please set both consists and trainset directories")
            return
        ...
        if resolve_mode == 'selected':
            ...
            if not selected_file:
                messagebox.showerror(
                    "Error", "No consist file selected. Please select a file from the consist files list.")
                return
    ...
    def _run_resolver_for_file(self, consist_file, trainset_dir):
        ...
        try:
            ...
            def _start_worker():
                try:
                    self._resolver_file_worker(
                        temp_dir, consist_file, trainset_dir)
                except Exception as e:
                    self.message_queue.put(
                        ('log', f"Error in resolver worker: {e}"))
            ...
            messagebox.showerror(
                "Error", f"Failed to prepare single file resolution: {e}")
        ...
        def _dir_worker():
            try:
                self._run_resolver_thread(
                    consists_dir, trainset_dir, refresh_after=True)
            except Exception as e:
                self.message_queue.put(
                    ('log', f"Error running resolver for directory: {e}"))
    ...
    def _resolver_file_worker(self, temp_dir, original_consist_path, trainset_dir):
        ...
        try:
            return_code = self._run_resolver_thread(
                temp_dir, trainset_dir, refresh_after=True)

            # Copy the resolved file back to original location if resolver succeeded
            if return_code == 0:
                resolved_temp_file = Path(
                    temp_dir) / Path(original_consist_path).name
                self.message_queue.put(
                    ('log', f"Checking for resolved temp file: {resolved_temp_file}"))
                if resolved_temp_file.exists():
                    try:
                        self.message_queue.put(
                            ('log', f"Copying resolved file from {resolved_temp_file} to {original_consist_path}"))
                        shutil.copy2(str(resolved_temp_file),
                                     original_consist_path)
                        self.message_queue.put(
                            ('log', f"Resolved file copied back to: {original_consist_path}"))

                        # Verify the copy worked and refresh missing count
                        if Path(original_consist_path).exists():
                            self.message_queue.put(
                                ('log', f"Verified: Original file exists at {original_consist_path}"))
                            try:
                                # Refresh the missing count for this specific file on the main thread
                                self._refresh_single_file_missing_count(
                                    original_consist_path)
                                ...
                                if hasattr(self, 'current_consist_file') and self.current_consist_file and str(self.current_consist_file) == str(original_consist_path):
                                    self.message_queue.put(
                                        ('log', f"Refreshing consist viewer for updated file: {original_consist_path}"))
                                    self.message_queue.put(
                                        ('refresh_current_consist', None))
                            except Exception as e:
                                self.message_queue.put(
                                    ('log', f"Error refreshing after resolver: {e}"))
                        else:
                            self.message_queue.put(
                                ('log', f"Error: Original file not found after copy at {original_consist_path}"))
                    except Exception as e:
                        self.message_queue.put(
                            ('log', f"Error copying resolved file back: {e}"))
                else:
                    self.message_queue.put(
                        ('log', f"Warning: Resolved temp file not found: {resolved_temp_file}"))
                    try:
                        contents = list(Path(temp_dir).iterdir())
                        self.message_queue.put(
                            ('log', f"Temp directory contents: {[str(p) for p in contents]}"))
                    except Exception as e:
                        self.message_queue.put(
                            ('log', f"Error listing temp directory: {e}"))
            else:
                self.message_queue.put(
                    ('log', f"Resolver failed with code {return_code}, not copying back resolved file"))

        except Exception as e:
            self.message_queue.put(
                ('log', f"Error in single-file resolver worker: {e}"))
        finally:
    ...
    def _run_resolver_thread(self, consists_dir, trainset_dir, refresh_after=True):
        ...
        try:
            ...
            cmd = [self.venv_python_path, self.resolver_script_path,
                   consists_dir, trainset_dir]
            ...
            self.message_queue.put(
                ('log', f"Running resolver command: {' '.join(cmd)}"))
            self.message_queue.put(('log', "Resolver started..."))

            # Disable button during processing
            self.message_queue.put(
                ('button_state', ('disabled', 'Running...')))
                ...
                if output:
                    ...
                    if 'Found' in stripped and 'consist files' in stripped:
                        try:
                            import re
                            match = re.search(
                                r'Found (\d+) consist files', stripped)
                            if match:
                                total_files = int(match.group(1))
                                self.message_queue.put(
                                    ('resolver_progress_update', (0, total_files, 'Scanning files...')))
                        except Exception:
                            pass
                        ...
                        try:
                            import re
                            match = re.search(
                                r'with (\d+) asset references', stripped)
                            if match:
                                total_entries = int(match.group(1))
                                self.message_queue.put(
                                    ('resolver_progress_update', (0, total_entries, 'Processing assets...')))
                        except Exception:
                            pass
                        ...
                        try:
                            import re
                            match = re.search(
                                r'Processed (\d+) entries', stripped)
                            if match:
                                processed = int(match.group(1))
                                self.message_queue.put(
                                    ('resolver_progress_update', (processed, total_entries or processed, 'Completing...')))
                        except Exception:
                            pass
            ...
            self.message_queue.put(
                ('log', f"Resolver finished with return code: {return_code}"))

            # Re-enable button
            self.message_queue.put(
                ('button_state', ('normal', 'Run Resolver')))
            ...
            self.message_queue.put(
                ('log', f"Error running resolver: {str(e)}"))
            self.message_queue.put(
                ('button_state', ('normal', 'Run Resolver')))
    ...
    def _refresh_single_file_missing_count(self, file_path):
        ...
        try:
            ...
            try:
                ...
                err = str(ex)
                self.log_message(
                    f"Error calculating missing count for {file_path}: {err}")
            ...
            try:
                ...
                if self.consist_files_tree.exists(file_path_str):
                    ...
                    # Update the tree item
                    self.consist_files_tree.item(
                        file_path_str, values=(display_missing,), tags=(tag,))
                    ...
                    self.log_message(
                        f"Updated missing count for {Path(file_path).name}: {display_missing} (tag: {tag})")
                else:
                    self.log_message(
                        f"Tree item not found for: {file_path_str}")
            except Exception as e:
                self.log_message(
                    f"Error updating tree item for {file_path}: {e}")

        except Exception as e:
            self.log_message(
                f"Error in _refresh_single_file_missing_count: {e}")
