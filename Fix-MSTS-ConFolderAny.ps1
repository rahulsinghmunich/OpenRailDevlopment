...
function Try-FastPath([string]$kind,[string]$shape,[string]$folder,[object[]]$local,[object[]]$global){
    ...
    $exLocal = @($local | Where-Object { (Get-NormKey $_.Folder ($_.Shape ?? $_.Name)) -eq $needKey })
    if($exLocal.Count -gt 0){
        $best = $exLocal[0]
        Write-Log 'INFO' ("FastPath hit: LocalExact for '{0}' in '{1}' -> {2}/{3}" -f $shape,$folder,($best.Shape ?? $best.Name),$best.Folder)
        return $best
    }

    # 2) Exact in global
    $exGlob = @($global | Where-Object { (Get-NormKey $_.Folder ($_.Shape ?? $_.Name)) -eq $needKey })
    if($exGlob.Count -gt 0){
        $best = $exGlob[0]
        Write-Log 'INFO' ("FastPath hit: GlobalExact for '{0}' in '{1}' -> {2}/{3}" -f $shape,$folder,($best.Shape ?? $best.Name),$best.Folder)
        return $best
    }
...
function Try-FastPath([string]$kind,[string]$shape,[string]$folder,[object[]]$local,[object[]]$global){
    ...
    # 1) Exact in same folder
    $exLocal = @($local | Where-Object { (Get-NormKey $_.Folder $_.Name) -eq $needKey })
    if ($exLocal.Count -gt 0) {
        $best = $exLocal[0]
        Write-Log 'INFO' ("FastPath hit: LocalExact for '{0}' in '{1}' -> {2}/{3}" -f $shape,$folder,$best.Name,$best.Folder)
        return $best
    }

    # 2) Exact in global
    $exGlob = @($global | Where-Object { (Get-NormKey $_.Folder $_.Name) -eq $needKey })
    if ($exGlob.Count -gt 0) {
        $best = $exGlob[0]
        Write-Log 'INFO' ("FastPath hit: GlobalExact for '{0}' in '{1}' -> {2}/{3}" -f $shape,$folder,$best.Name,$best.Folder)
        return $best
    }
